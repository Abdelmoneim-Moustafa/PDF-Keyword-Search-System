...
def _fetch(url: str, session_timeout: int) -> tuple[bytes | None, str]:
    """
    ...
    """
    urls = [url] + _get_alternate_urls(url)
...
def process_one_url(url: str, raw_keyword: str, search_mode: str,
    ...
    # ── Download ──────────────────────────────────────────────────
    content, dl_status = _fetch(url, session_timeout)
    if content is None:
    ...
    if _is_html_not_found(norm_text):
        return row(URL_Status=3, URL_Search_Status="Done",
...
""", unsafe_allow_html=True)
                ...
                if start_btn and not st.session_state.running:
                    ...
                    if enable_retry and p1_err and st.session_state.running:
                        ...
                        err_keys = {
                            (r.get("URL", ""), r.get("Keyword", "")) for r in p1_err
                        }
                        results = [r for r in results
                                   if (r.get("URL",""), r.get("Keyword","")) not in err_keys]
                        total  += len(p1_err)
                        _log("⏳ 15 s cooldown before Pass 2…")
                        time.sleep(15)
                        p2_res, still_failed = _run_pass(p1_err, "Pass2", extra_delay=1.0)
